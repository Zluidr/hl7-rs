...
- [x] **Status:** DONE (Jul 30, 2026) — `cargo publish --dry-run -p hl7-arrow` passes; `cargo doc -p hl7-arrow --no-deps` zero warnings; `cargo deny check` clean workspace-wide.
- **Dependency:** HT-T10.2
...
- **Completed (Jul 30, 2026):** running `cargo publish --dry-run -p hl7-arrow` surfaced two cascading publish-readiness gaps unrelated to `hl7-arrow` itself, since `cargo publish` resolves path dependencies against published registry state, not the local workspace: (1) `hl7-arrow`'s dev-dependency `hl7-mllp = "^0.2.0"` had never been published — crates.io only had `0.1.0`/`0.0.1` despite the local `Cargo.toml` bump back at v0.1.2; (2) `hl7-v2`'s public `all_segments()` (added in the HT-T10.2 work) was never accompanied by a version bump, so the published `0.0.1` predates it. With explicit user confirmation at each step: published `hl7-mllp` `0.2.0` to crates.io for real; bumped `hl7-v2` to `0.1.0` (new `crates/hl7-v2/CHANGELOG.md`), updated the root `[workspace.dependencies]` pin, verified `cargo build`/`test`/`clippy -D warnings`/`fmt --check` stayed clean workspace-wide, then published `hl7-v2` `0.1.0` to crates.io for real. `cargo publish --dry-run -p hl7-arrow` now passes cleanly.
